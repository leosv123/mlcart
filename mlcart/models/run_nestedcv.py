...
class NestedCV():
    ...
    def fit(self, X, y):
        ...
        outer_cv = KFold(n_splits=self.outer_kfolds, shuffle=True, random_state=0)
        inner_cv = KFold(n_splits=self.inner_kfolds, shuffle=True,random_state=0)
        ...
        for (i, (train_index, test_index)) in enumerate(outer_cv.split(X, y)):
            ...
            for (j, (train_index_inner, test_index_inner)) in enumerate(inner_cv.split(X_train_outer, y_train_outer)):
                ...
                def _parallel_fitting(X_train_inner, X_test_inner, y_train_inner, y_test_inner, param_dict):
                    ...
                    inner_grid_score, inner_pred = self._predict_and_score(
            ...
            best_inner_score, best_inner_params = self._best_of_results(
                search_scores)
